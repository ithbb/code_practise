...
这道题目其实是Regular Expression Matching的简化版，在这里'?'相当于那边的'.'，而'*'相当于那边的'.*'，
因为这里'*'就可以代替任何字符串，不需要看前面的字符，所以处理起来更加简单。
brute force的方法就不重新列举代码了，有兴趣实现的朋友可以参考一下Regular Expression Matching，
代码结构一样，只是处理情况变一下就可以，不过leetcode过不了（因为超时）。
我们主要还是说一下动态规划的方法。
跟Regular Expression Matching一样，还是维护一个假设我们维护一个布尔数组res[i],
代表s的前i个字符和p的前j个字符是否匹配(这里因为每次i的结果只依赖于j-1的结果，所以不需要二维数组，
只需要一个一维数组来保存上一行结果即可），递推公式分两种情况：
